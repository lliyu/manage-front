	...
	export default {
		...
		methods: {
			...
			getUsers () {
				...
				http.get("/api/user/list", params).then(function (response){
          ...
          this.delUsers()
			...
			async delUsers () {
				...
				let data =await http.post('api/user/delete', params)
				if(!data.data) {
					return
				}
				if (data.data.status === 200) {
			...
			async getCboData () {
				let _this = this
				let data = await http.get('api/role/findAllRoles')
				if(!data.data) {
					return
				}
				if (data.data.status === 200) {
      ...
      handleCurrentChange (val) {
        ...
        this.editUserStatus(status, id)
			...
			async editUserStatus (status,id) {
				...
				let data = await http.post('api/user/editUserStatus', params)
