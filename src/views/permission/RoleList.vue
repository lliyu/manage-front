	...
	export default {
		...
		methods: {
			...
			getFormData () {
				...
				http.get('api/role/list', params).then(function (res) {
					if (res.status === 200) {
           ...
           name: _this.ruleForm.name,
				...
				let data = await http.post('api/role/add', params)
          ...
          this.delRoles()
			...
			async delRoles () {
				...
				let data =await http.post('api/role/delete', params)
			...
			async roleEdit(val) {
				...
				if (data.data.status === 200) {
					 ...
					 let pidData = await http.get('api/role/rolePermission', params)
        ...
        let params = {
				...
				let data = await http.post('api/role/addRolesPermis', params)
				if (data.data.status === 200) {
